...
const ANIMATION_TEMPLATES = [
  // Tenor Discord Welcome Trending Collection (from user's link https://tenor.com/view/discord-welcome-gif-23878933)
  {
...
export function WelcomeHub({ userRole = 'owner' }) {
  ...
  // 1. Fetch connected Guilds with server isolation
  const fetchGuilds = useCallback(async () => {
    setGuildsLoading(true);
    try {
      const queryRole = isMasterOwner ? 'owner' : (userRole || 'user');
      const queryUser = currentUser?.username || '';
      const res = await fetch(`${API}/api/bot/guilds?user_role=${encodeURIComponent(queryRole)}&username=${encodeURIComponent(queryUser)}`);
      if (res.ok) {
  ...
  }, [isMasterOwner, userRole, currentUser?.username, selectedGuildId]);
  ...
  return (
    <div className="space-y-6 pb-16">
          ...
          {isMasterOwner ? (
            <>
              ...
              <button
                ...
                className="px-3.5 py-2.5 rounded-xl bg-purple-950/80 hover:bg-purple-900 border border-purple-500/40 text-purple-200 font-mono text-xs flex items-center gap-1.5 shadow-lg transition-all"
              >
                <Crown className="w-3.5 h-3.5 text-yellow-400" />
                <span>👑 Owner VIP Keys</span>
              </button>
            </>
          ) : (
            <div className="px-4 py-2.5 rounded-xl bg-slate-950/80 border border-emerald-500/30 text-emerald-300 font-mono text-xs flex items-center gap-2 shadow-inner">
              <Lock className="w-3.5 h-3.5 text-emerald-400 shrink-0" />
              <span>🔒 Private Server View: <strong>{currentGuild?.name || 'Your Server'}</strong></span>
            </div>
          )}
      ...
      {/* Side-By-Side Plan Overview (Free Plan vs VIP 150 BDT Plan) */}
        ...
        <div className={`p-5 rounded-2xl border transition-all ${
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-sm font-bold text-slate-200 font-mono">🟢 FREE PLAN (0 BDT)</h3>
            </div>
            {!isVip && (
          ...
          <div className="mt-3 space-y-1.5 text-xs font-mono">
            ...
            <div className="flex items-center gap-2 text-slate-500 line-through">
              ...
              <span>Tenor & Custom Animated GIFs / Video Banners (Locked)</span>
            </div>
            <div className="flex items-center gap-2 text-slate-500 line-through">
              <X className="w-3.5 h-3.5 text-slate-600" />
              <span>Welcome Join Log Channel for Staff (Locked)</span>
            </div>
            <div className="flex items-center gap-2 text-slate-500 line-through">
              <X className="w-3.5 h-3.5 text-slate-600" />
              <span>Dedicated Ban Log Channel with Reason & Mod (Locked)</span>
            </div>
            <div className="flex items-center gap-2 text-slate-500 line-through">
              <X className="w-3.5 h-3.5 text-slate-600" />
              <span>Custom 5x Emoji Selector (Locked)</span>
            </div>
          </div>
        </div>

        {/* Plan 2: VIP Premium Plan (150 BDT) */}
        <div className={`p-5 rounded-2xl border transition-all ${
          isVip
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-sm font-bold text-amber-300 font-mono">👑 GMX VIP PREMIUM (150 BDT)</h3>
            </div>
            {isVip ? (
              ...
              <div className="flex items-center gap-1.5">
                <button
                  ...
                  className="text-[10px] font-mono px-2.5 py-1 rounded-lg bg-gradient-to-r from-amber-500 to-yellow-400 text-slate-950 font-extrabold hover:from-amber-400 hover:to-yellow-300 shadow-[0_0_15px_rgba(245,158,11,0.4)]"
                >
                  ⚡ BUY (150 BDT)
                </button>
                <button
                  ...
                  className="text-[10px] font-mono px-2 py-1 rounded-lg bg-slate-800 text-amber-300 hover:bg-slate-700 border border-amber-500/30"
                >
                  ENTER KEY
                </button>
              </div>
          ...
          <p className="text-xs text-slate-400 mt-1">Full access package for 150 Taka: animated GIFs, ban logs, join logs & custom emojis.</p>
          <div className="mt-3 space-y-1.5 text-xs font-mono">
            <div className="flex items-center gap-2 text-amber-200">
              <Sparkles className="w-3.5 h-3.5 text-amber-400" />
              <span>Tenor Discord Welcome GIFs & Custom MP4 Video URLs</span>
            </div>
            <div className="flex items-center gap-2 text-amber-200">
              <Check className="w-3.5 h-3.5 text-emerald-400" />
              <span><strong>📋 Welcome Log Channel:</strong> Full join audit logs for staff</span>
            </div>
            <div className="flex items-center gap-2 text-amber-200">
              <Check className="w-3.5 h-3.5 text-emerald-400" />
              <span><strong>🔨 Ban Log Channel:</strong> Instant ban embeds with mod & reason</span>
            </div>
            <div className="flex items-center gap-2 text-amber-200">
              <Check className="w-3.5 h-3.5 text-emerald-400" />
              <span><strong>🚪 Leave / Kick Log Channel:</strong> Tracks departures in real time</span>
            </div>
            <div className="flex items-center gap-2 text-amber-200">
              <Sparkles className="w-3.5 h-3.5 text-amber-400" />
              <span><strong>✨ 5x Custom Emojis:</strong> Rules, Chat, Announce, Headline, Slogan</span>
            </div>
            <div className="flex items-center gap-2 text-amber-200">
              <Sparkles className="w-3.5 h-3.5 text-amber-400" />
              <span>Advanced Anti-Nuke Shield & Automated License Delivery</span>
            </div>
          </div>
        </div>
      </div>
      ...
      {/* 👑 VIP 150 BDT PURCHASE & LICENSE KEY MODAL 👑 */}
      {showVipModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/85 backdrop-blur-md overflow-y-auto">
          <div className="relative w-full max-w-xl rounded-3xl bg-gradient-to-b from-slate-900 via-slate-950 to-black border-2 border-amber-500/50 shadow-[0_0_50px_rgba(245,158,11,0.3)] p-6 sm:p-8 space-y-6 my-8 animate-in fade-in zoom-in duration-200">
            {/* Close Button */}
            <button
              onClick={() => {
                setShowVipModal(false);
            ...
            <div className="text-center space-y-2">
              <div className="w-14 h-14 rounded-2xl bg-gradient-to-tr from-amber-500 to-yellow-300 mx-auto flex items-center justify-center shadow-[0_0_25px_rgba(245,158,11,0.5)]">
